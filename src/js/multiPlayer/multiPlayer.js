...
define(['game', 'audio', 'mWorld', 'tank', 'multiPlayer_draw'], function(
    game,
    audio,
    mWorld,
    tank,
    ...
    const displayMultiplayer = () => {
        ...
        for (let key in game.users) {
            ...
            if (document.getElementById(userId)) {
                if (
                    parseInt(document.getElementById(userIdScore).innerHTML) !==
                    user.points
                ) {
                    document.getElementById(userIdScore).innerHTML =
                        user.points;
                }

                if (
                    parseInt(document.getElementById(userIdLives).innerHTML) !==
                    user.lives
                ) {
                    if (user.lives < 0) {
                        mpDisplay.innerHTML = '';
                    } else {
                        document.getElementById(userIdLives).innerHTML =
                            user.lives;
                    }
                }
