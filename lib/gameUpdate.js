...
const checkBulletCollision = (x, y, dir, gameState, i) => {
  ...
  for (k; k < playersLen; k++) {
    ...
    if ((x === p_x || x === p_x + 1 || x === p_x - 1) && (y === p_y || y === p_y + 1 || y === p_y - 1)) {
      ...
      gameState.game.users[gameState.players[i].id].points += 1;
      // gameState.game.users[gameState.players[i].id].explosion = true;
      return true;
    }
  }
  return false;
...
const gameUpdate = (gameState) => {
  ...
  for (let i = 0; i < playersLen; i++) {
    if (gameState.players[i].bulletFired) {
      ...
      if (checkBulletCollision(gameState.players[i].bullet.x, gameState.players[i].bullet.y, gameState.players[i].bullet.dir, gameState, i)) {
