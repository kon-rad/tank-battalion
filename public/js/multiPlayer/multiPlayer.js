...
define(['game', 'events', 'audio', 'mWorld', 'tank', 'multiPlayer_draw'], function (game, events, audio, mWorld, tank, multiPlayer_draw) {
	...
	var displayMultiplayer = function displayMultiplayer() {
		...
		for (var user in game.mpGame.users) {
			...
			mpDisplay.innerHTML += '<div class="mpS__user"><span>user:</span>' + obj[user].name + '<div id="mpS_' + obj[user].id + '"class="mpS__score"><span>score</span>' + obj[user].points + '</div>' + '<div class="mpS__lives"><span>lives</span>' + obj[user].lives + '</div></div>';
		}
	};
